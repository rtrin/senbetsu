...
import type { ClassificationRequest, CommandResponse, SavedSession, SavedTab } from './types';
import { isClassifiableUrl } from './utils';

export async function handleSaveAndGroup(): Promise<CommandResponse> {
  try {
    const tabs = await chrome.tabs.query({ currentWindow: true });
    const classifiable = tabs.filter((t) => t.id !== undefined && isClassifiableUrl(t.url));

    const requests: ClassificationRequest[] = classifiable.map((t) => ({
      tabId: t.id!,
      url: t.url!,
      title: t.title ?? '',
      description: '',
    ...
    const results = await classifyTabs(requests);
    if (results.length === 0) {
